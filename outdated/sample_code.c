		   ...
		   MPI_Comm comm)
{
  ...
  for (i=0; i<size; i++) { // i = j in script
    int t, f;
    t = (rank+i)%size; // rank = i in script
    f = (rank-i+size)%size; 
    // (i-j)_p in script (the "+size" is to avoid neg. values)
    // the (i-j)_p implements this internally
...
int main(int argc, char *argv[])
{
  ...
  count = 1; // S^i_j is a number in the simplest case 
  // (could also be array etc.)
  ...
  for (c=1; c<=count; c*=f,FLIP(f,2,5)) { 
    ...
    for (r=0,t=0; r<WARMUP+REPEAT; r++) {
      ...
      ALLTOALL(sendbuf,c,TUW_TYPE,testbuf,c,TUW_TYPE,MPI_COMM_WORLD);
      // plug in custom self-made all-reduce here (but only later after creating the benchmark setup)
  ...
  /* -------- TO DO --------
  - (is the present MPI_Allreduce, line 230, already the one to be benchmarked?
  - implement MPI_Reduce + MPI_Bcast Reference 
  - implement missing statistics (median, confidence intervals, stddev)
  - change powers of loop from 2 and 5 to 2 and 10
  - run on hydra (check process configuration (nodes, count) given on sheet)
  - plot results
